...
curl -s -X POST \
  ...
  -d '{
	"peers": ["localhost:7051","localhost:7056"],
	"chaincodeName":"trade",
	"chaincodePath":"github.com/trade",
	"chaincodeVersion":"v0"
}'
echo
echo
...
echo
curl -s -X GET \
  "http://localhost:4000/channels/mychannel/chaincodes/trade?peer=peer1&args=%5B%22query%22%2C%22a%22%5D&chaincodeVersion=v0" \
  -H "authorization: Bearer $ORG1_TOKEN" \
  -H "cache-control: no-cache" \
  -H "content-type: application/json" \
  -H "x-access-token: $ORG1_TOKEN"
